...
class TravelPlannerChat {
  ...
  async sendQuery(question) {
    ...
    try {
      ...
      this.lastResponse = result.answer;
  ...
  downloadPDF() {
    if (!this.lastResponse) {
      this.showError("No travel plan available to download.");
      return;
    }

    try {
      // Create a proper text file with travel plan content
      ...
      link.href = url;
      link.download = `travel_plan_${new Date().toISOString().split('T')[0]}.txt`;
      ...
      this.showError("Failed to download the travel plan. Please try again.");
    }
  }

  async clearChat() {
    if (confirm("Are you sure you want to clear the chat history? This will start a new conversation session.")) {
      try {
